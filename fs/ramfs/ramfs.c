...
static uint32_t ramfs_write(fs_node_t *node, const char *data, uint32_t size) {
    ...
    if (offset + to_write > RAMFS_FILE_MAX_SIZE - 1) {
        to_write = RAMFS_FILE_MAX_SIZE - 1 - offset;
    }

    memcpy(buf + offset, data, to_write);
...
static fs_node_t* ramfs_create(fs_node_t* dir, const char* name) {
    ...
    new_node->data = kmalloc(RAMFS_FILE_MAX_SIZE);

    memset(new_node->data, 0, RAMFS_FILE_MAX_SIZE);
...
void ramfs_init() {
    ...
    memset(ramfs_root_node, 0, sizeof(fs_node_t));

    char rootname[] = "ramroot";
    strcpy(ramfs_root_node->name, rootname);
