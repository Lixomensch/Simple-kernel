...
void kheap_init() {
    ...
    heap_current_top = start_addr + PAGE_SIZE;
...
void* kmalloc(size_t size) {
    ...
    if (last->free) {
        pages_needed = (size - last->size + PAGE_SIZE - 1) / PAGE_SIZE;
    } else {
        pages_needed = (size + sizeof(block_header_t) + PAGE_SIZE - 1) / PAGE_SIZE;
    }
    ...
    if (last->free) {
        ...
        expanded->size = (pages_needed * PAGE_SIZE) - sizeof(block_header_t);
        expanded->free = true;
        expanded->next = NULL;
        last->next = expanded;
    }
